...
function Backend(nuts, opts) {
    this.cacheId = 0;
    this.nuts = nuts;
    let cachePath = path.resolve(os.tmpdir(), 'nuts');
...
Backend.prototype.serveAsset = function(asset, req, res) {
    ...
    console.log("calling onBackendServeAsset : " + fullUrl)
    console.log("serving asset with id : " + asset.id + "\n")
    ...
    if (that.cache.has(cacheKey)) {
        console.log("asset [ " + asset.id + " ] found in cache : " + cacheKey)
